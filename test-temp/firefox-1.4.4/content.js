...
`;
  ...
  function makeBoard(canvas, mode, getBg) {
    ...
    var strokes = [], cur = null, drawing = false, _tool='pen', _eraserPos=null, _eraserRad=20;
    ...
    function eraseAt(px,py,lw) {
      ...
      strokes.forEach(function(s){
        if (s.tool==='text') {
          if (Math.hypot(s.sx-px,s.sy-py)>r) newStrokes.push(s);
          return;
        }
        if (s.tool==='image') {
          if (!(px>=s.sx&&px<=s.sx+s.w&&py>=s.sy&&py<=s.sy+s.h)) newStrokes.push(s);
          return;
        }
        if (s.pts) {
          // For polylines: remove only points near eraser, split if needed
          var kept=[], segments=[];
          for (var i=0;i<s.pts.length;i++) {
            var dist=Math.hypot(s.pts[i].x-px,s.pts[i].y-py);
            if (dist>r) kept.push(s.pts[i]);
            else {
              if (kept.length>=2) segments.push(kept.slice());
              kept=[];
            }
          }
          if (kept.length>=2) segments.push(kept);
          // Create strokes for each viable segment
          segments.forEach(function(seg,idx){
            newStrokes.push({tool:s.tool,color:s.color,lw:s.lw,pts:seg.slice()});
          });
        } else {
          // Shapes: check distance from center
          if (Math.hypot((s.sx+s.ex)/2-px,(s.sy+s.ey)/2-py)>r*2) newStrokes.push(s);
        }
      });
      strokes=newStrokes;
      redraw();
    ...
    function onMove(tool,lw,e) {
      _tool=tool;
      var p=getCoord(e);
      if(tool==='eraser'){_eraserRad=lw*4+12;_eraserPos=p;redraw();if(!drawing)return;eraseAt(p.x,p.y,lw);return;}
      if(tool==='pan') {
    ...
    function onUp(tool) {
      ...
      if(cur){saveState();strokes.push(cur); if(tool==='magic')scheduleFade(cur); cur=null;}
      redraw();
    }
    // Undo/Redo system
    var undoStack=[], redoStack=[], MAX_UNDO=50;
    function saveState(){undoStack.push(strokes.slice());if(undoStack.length>MAX_UNDO)undoStack.shift();redoStack=[];}
    function undo(){
      if(undoStack.length===0)return;
      redoStack.push(strokes.slice());
      strokes=undoStack.pop();cur=null;selectedStrokes=[];selectRect=null;redraw();
    }
    function redo(){
      if(redoStack.length===0)return;
      undoStack.push(strokes.slice());
      strokes=redoStack.pop();cur=null;selectedStrokes=[];selectRect=null;redraw();
    }
    function pushText(color,lw,pos,text){saveState();strokes.push({tool:'text',color,lw,sx:pos.x,sy:pos.y,text});redraw();}
    function addImage(dataUrl,x,y,w,h){saveState();strokes.push({tool:'image',sx:x,sy:y,w,h,dataUrl});redraw();}
    function clearAll(){saveState();strokes=[];cur=null;selectedStrokes=[];selectRect=null;redraw();}
    function getStrokes(){return strokes.slice();}
    function setStrokes(arr){saveState();strokes=arr?arr.slice():[];cur=null;selectedStrokes=[];selectRect=null;strokes.forEach(function(s){if(s.tool==='image'&&!imageCache.has(s)){var img=new Image();img.src=s.dataUrl;imageCache.set(s,img);}});redraw();}
    document.addEventListener('keydown',function(e){
      ...
      if (e.key==='Delete'||e.key==='Backspace'){saveState();strokes=strokes.filter(function(s){return selectedStrokes.indexOf(s)<0;});selectedStrokes=[];redraw();e.preventDefault();}
  ...
  function buildGroupButtons(btns, container, isFlyout) {
    btns.forEach(function(key) {
      if (key === 'colors') {
        ...
        cpInput.type = 'color';
        cpInput.value = ovState.color;
        cpInput.style.cssText = 'width:28px;height:28px;border:none;border-radius:7px;cursor:pointer;padding:0;background:none;vertical-align:middle;margin:1px;';
        cpInput.title = 'Custom color';
        cpInput.addEventListener('input', function(){
          document.querySelectorAll('.AN_clr').forEach(function(b){ b.classList.remove('AN_active'); });
          ovState.color = this.value;
        });
        cb.appendChild(cpInput);
  ...
  function makeAnBtn(key) {
    var defs = {
      ...
      redo:   { cls:'AN_act',  id:'AN_redo',   icon:'↪️', title:'Redo'      },
  ...
  function reattachBarListeners() {
    ...
    var redoEl = document.getElementById('AN_redo');
    if (redoEl) redoEl.addEventListener('click', function(){ OV.redo(); closeFlyouts(); });

    var clearEl = document.getElementById('AN_clear');
    if (clearEl) clearEl.addEventListener('click', function(){
      AN_confirm('সব অ্যানোটেশন মুছে ফেলবো?', function(){ OV.clearAll(); });
      closeFlyouts();
    });
  ...
  // Ctrl+Z/Y handled inside makeBoard — no duplicate needed
  ...
  var DEFAULT_WB_GROUPS = [
    ...
    { id:'g5', name:'Actions',  icon:'⚡', mode:'inline',  btns:['undo','redo','clear','zoom'] },
  ...
  function buildWbGroupButtons(btns, container) {
    btns.forEach(function(key) {
      if (key === 'colors') {
        ...
        // Custom color picker for whiteboard
        var cpInput = document.createElement('input');
        cpInput.type = 'color';
        cpInput.value = wbState.color;
        cpInput.style.cssText = 'width:28px;height:28px;border:none;border-radius:7px;cursor:pointer;padding:0;background:none;vertical-align:middle;';
        cpInput.title = 'Custom color';
        cpInput.addEventListener('input', function(){
          document.querySelectorAll('.AN_wbClr').forEach(function(b){ b.classList.remove('AN_wbClrActive'); });
          wbState.color = this.value;
        });
        cb.appendChild(cpInput);
        container.appendChild(cb);
        return;
  ...
  function makeWbBtn(key) {
    var defs = {
      ...
      redo:   { id:'AN_wbRedo',  icon:'↪️', act:true },
  ...
  function reattachWbListeners() {
    ...
    var redoEl = document.getElementById('AN_wbRedo');
    if (redoEl) redoEl.addEventListener('click', function(){ WB.redo(); });
  ...
  function AN_applyLang(){
    ...
    pv=ge('AN_wbRedo');  if(pv) pv.title=t('wbRedo');
