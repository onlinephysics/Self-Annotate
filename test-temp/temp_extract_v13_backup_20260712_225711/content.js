...
`;
  ...
  var wbState={tool:'pen',color:'#ef4444',lw:6,bg:'#9ca3af',isFull:false,isMin:false,textPos:null,savedL:'',savedT:'',savedW:'',savedH:'',zoom:1};
  ...
  function buildWbGroupButtons(btns, container) {
    btns.forEach(function(key) {
      ...
      if (key === 'zoom') {
        ...
        zmOut.addEventListener('click', function(){ wbState.zoom = Math.max(0.25, wbState.zoom - 0.25); wbFit(); updateZoomDisplay(); });
        var zmIn = document.createElement('button');
        zmIn.className = 'AN_wbZoomBtn'; zmIn.textContent = '+';
        zmIn.addEventListener('click', function(){ wbState.zoom = Math.min(5, wbState.zoom + 0.25); wbFit(); updateZoomDisplay(); });
        ...
        zmSl.addEventListener('input', function(){ wbState.zoom = +zmSl.value / 100; wbFit(); updateZoomDisplay(); });
  ...
  var WB=makeBoard(wbCanvas,'window',function(){return wbState.bg;},function(){return wbState.zoom;});
  ...
  function applyWbZoom(){
    var z=wbState.zoom||1;
    wbCanvas.style.transform='scale('+z+')';
  }
  ...
  wbCanvas.addEventListener('touchstart',function(e){
    ...
    if(e.touches.length===2){
      ...
      wbTouchDist=Math.sqrt(dx*dx+dy*dy);
      return;
    }
  ...
  wbCanvas.addEventListener('touchmove',function(e){
    ...
    if(e.touches.length===2){
      ...
      if(wbTouchDist>0){
        var newZoom=Math.max(0.25,Math.min(5,wbState.zoom*(dist/wbTouchDist)));
        if(newZoom!==wbState.zoom){wbState.zoom=newZoom;wbFit();updateZoomDisplay();}
      }
      wbTouchDist=dist;
      return;
    }
  ...
  wbCanvas.addEventListener('touchend',function(){
    wbTouchDist=0;
  ...
  function exportPDF(indices){
    ...
    function next(){
      if(i>=indices.length){
        ...
        txt.textContent='✅ PDF তৈরী হচ্ছে…';
        buildPdfHtml(images);
        return;
      }
      var idx=indices[i];
      ...
      tmpCv.width=Math.round(w*0.5); tmpCv.height=Math.round(h*0.5);
      var tmpCtx=tmpCv.getContext('2d');
      tmpCtx.scale(0.5,0.5);
      tmpCtx.fillStyle=wbState.bg; tmpCtx.fillRect(0,0,w,h);
      var savedStrokes=WB.getStrokes();
      WB.setStrokes(wbPages[idx]); WB.redraw();
      tmpCtx.drawImage(wbCanvas,0,0);
      WB.setStrokes(savedStrokes); WB.redraw();
      images.push(tmpCv.toDataURL('image/jpeg',0.92));
      i++; setTimeout(next,30);
    }
    next();
  }
  function buildPdfHtml(images){
    var html='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Whiteboard PDF</title>';
    html+='<style>body{margin:0;padding:10mm;background:#e2e8f0;}';
    html+='.page{background:#fff;box-shadow:0 2px 12px rgba(0,0,0,.12);margin:0 auto 10mm;';
    html+='width:190mm;min-height:277mm;display:flex;align-items:center;justify-content:center;overflow:hidden;}';
    html+='.page img{width:100%;height:auto;display:block;}';
    html+='@media print{body{padding:0;background:#fff;}.page{box-shadow:none;margin:0;page-break-after:always;width:100%;min-height:100vh;}}';
    html+='</style></head><body>';
    images.forEach(function(src){html+='<div class="page"><img src="'+src+'"></div>';});
    html+='</body></html>';
    var blob=new Blob([html],{type:'text/html'});
    var url=URL.createObjectURL(blob);
    var w=window.open(url);
    if(w){setTimeout(function(){URL.revokeObjectURL(url);document.getElementById('AN_expModal').classList.remove('AN_show');},100);}
    else{
      var a=document.createElement('a');a.href=url;a.download='whiteboard-pages.html';
      document.body.appendChild(a);a.click();
      setTimeout(function(){document.body.removeChild(a);URL.revokeObjectURL(url);document.getElementById('AN_expModal').classList.remove('AN_show');},100);
    }
  }
  ...
  function updateZoomDisplay(){
    ...
    if(el)el.textContent=Math.round(wbState.zoom*100)+'%';
    var sl=document.getElementById('AN_wbZoomSlider');
    if(sl)sl.value=Math.round(wbState.zoom*100);
  }
  var _zoomVis=false;
  ...
  function wbRestoreFromData(data){
    ...
    if(data.zoom)wbState.zoom=Math.max(0.25,Math.min(5,data.zoom));
